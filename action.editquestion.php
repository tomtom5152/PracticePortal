...
$idquestions = isset($params['idquestions']) ? $params['idquestions'] : $this->Redirect($id, 'addquestion', $returnid);
...
if( isset($params['submit'])) {
    ...
    if($error !== FALSE) {
        ...
        $sql = 'UPDATE '.cms_db_prefix().'module_practiceportal_questions '
                .'SET question = ?, content = ?, idlocations = ?, idconditions = ? '
                .'WHERE idquestions = ?';
        $dbr = $db->Execute($sql, array($question, $content, $idlocations, $idconditions, $idquestions));
        ...
        if( !$dbr )
	  {
        ...
        $params = array('tab_message'=> 'editsuccessful', 'active_tab' => 'questions');
        $this->Redirect($id, 'defaultadmin', $returnid, $params);
    }
} else {
    $sql = 'SELECT * FROM '.cms_db_prefix().'module_practiceportal_questions WHERE idquestions=?';
    $row = $db->GetRow($sql, array($idquestions));
    
    if($row) {
        // Reassign the variables with those from the database
        $question  = isset($params['question']) ? $params['question'] : $row['question'];
        $content = isset($params['content']) ? $params['content'] : $row['content'];
        $idlocations = isset($params['idlocations']) ? $params['idlocations'] : json_decode($row['idlocations']);
        $idconditions = isset($params['idconditions']) ? $params['idconditions'] : array_flip(json_decode($row['idconditions'], true));
    }
}

// Misc smarty
$smarty->assign('title_questions', $this->Lang('title_questions_edit'));
...
$smarty->assign('hidden', $this->CreateInputHidden($id, 'idquestions', $idquestions));
